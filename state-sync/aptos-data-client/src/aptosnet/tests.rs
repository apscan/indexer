...
use crate::aptosnet::logging::LogEntry::{StorageServiceRequest, StorageServiceResponse};
...
use storage_service_types::responses::{
    ...
    StorageServiceResponse, TransactionsWithProofResponse,
};
...
async fn disable_compression() {
    ...
    assert_matches!(request, DataRequest::GetStorageServerSummary);

    let summary = mock_storage_summary(200);
    response_sender.send(Ok(DataResponse::StorageServerSummary(summary)));
...
async fn all_peer_request_selection() {
    ...
    let server_version_request = StorageServiceRequest::new(data_request, true);
    assert_matches!(
    ...
    let output_data_request = StorageServiceRequest::new(data_request, true);
    assert_matches!(
    ...
    tokio::spawn(async move {
        while let Some((_, _, request, response_sender)) = mock_network.next_request().await {
            ...
            let storage_response =
                StorageServiceResponse::new(data_response, request.use_compression).unwrap();
            response_sender.send(Ok(storage_response));
        }
    });

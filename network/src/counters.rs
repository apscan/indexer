...
fn analyze_compression(data: Vec<u8>) {
    ...
    // Analyze snappy
    let (compress_time, decompress_time, relative_size) = analyze_snappy(data.clone());
    println!(
    ...
    // Analyze lz4
    for compression_level in 0..17 {
        let (compress_time, decompress_time, relative_size) =
            analyze_lz4(data.clone(), compression_level);
        println!(
            "{:?} >>>>> LZ4: Compress time: {:?}, Decompress time: {:?}, Resulting data size (%): {:?}",
            raw_data_length, compress_time, decompress_time, relative_size
        );
    }
}
...
fn analyze_lz4(raw_data: Vec<u8>, compression_level: u64) -> (Duration, Duration, f64) {
    let raw_data_length = raw_data.len();

    // Compress the input
    let compression_mode = CompressionMode::HIGHCOMPRESSION(compression_level as i32);
    let compress_start = Instant::now();
    let compressed = compress(&raw_data, Some(compression_mode), true).unwrap();
    let compress_duration = compress_start.elapsed();
    let compressed_data_length = compressed.len();

...
| load_test.config.accounts_per_client | int | `12` | The number of accounts to submit transactions from per client |
| load_test.config.duration | int | `300` | Duration of the load test in seconds |
| load_test.config.enableBurst | bool | `false` | Whether to enable burst mode and not wait for transactions to finalize |
...
| load_test.config.workers_per_ac | int | `64` | The number of workers per client |

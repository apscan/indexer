...
export class AptosClient {
  ...
  async getAccount(accountAddress: MaybeHexString): Promise<Gen.AccountData> {
    return await this.client.accounts.getAccount({ address: HexString.ensure(accountAddress).hex() });
  }
  ...
  async getAccountTransactions(
    ...
    return await this.client.transactions.getAccountTransactions({
      address: HexString.ensure(accountAddress).hex(),
  ...
  async getAccountModules(
    ...
    return await this.client.accounts.getAccountModules({
      address: HexString.ensure(accountAddress).hex(),
      ledgerVersion: query?.ledgerVersion?.toString(),
    });
  }
  ...
  async getAccountModule(
    ...
    return await this.client.accounts.getAccountModule({
  ...
  async getAccountResources(
    ...
    return await this.client.accounts.getAccountResources({
      address: HexString.ensure(accountAddress).hex(),
      ledgerVersion: query?.ledgerVersion?.toString(),
    });
  }
  ...
  async getAccountResource(
    ...
    return await this.client.accounts.getAccountResource({
      resourceType: moveStructTagToParam(resourceType),
      address: HexString.ensure(accountAddress).hex(),
      ledgerVersion: query?.ledgerVersion?.toString(),
    });
  }
  ...
  async createSigningMessage(request: Gen.EncodeSubmissionRequest): Promise<string> {
    return await this.client.transactions.encodeSubmission({ requestBody: request });
  }
  ...
  async getEventsByEventKey(eventKey: string): Promise<Gen.Event[]> {
    return await this.client.events.getEventsByEventKey({ eventKey: eventKey });
  }
  ...
  async getEventsByEventHandle(
    ...
    return await this.client.events.getEventsByEventHandle({
      address: HexString.ensure(address).hex(),
      eventHandle: moveStructTagToParam(eventHandleStruct),
      fieldName: fieldName,
      start: query?.start?.toString(),
      limit: query?.limit,
    });
  ...
  async submitTransaction(signedTxnRequest: Gen.SubmitTransactionRequest): Promise<Gen.PendingTransaction> {
    return await this.client.transactions.submitTransaction({ requestBody: signedTxnRequest });
  }
  ...
  async simulateTransaction(
    ...
    return await this.client.transactions.simulateTransaction({ requestBody: request });
  }
  ...
  async getTransactions(query?: { start?: BigInt; limit?: number }): Promise<Gen.Transaction[]> {
    return await this.client.transactions.getTransactions({
      start: query?.start?.toString(),
      limit: query?.limit,
    });
  }
  ...
  async getTransactionByHash(txnHash: string): Promise<Gen.Transaction> {
    return await this.client.transactions.getTransactionByHash({ txnHash: txnHash });
  }
  ...
  async getTransactionByVersion(txnVersion: BigInt): Promise<Gen.Transaction> {
    return await this.client.transactions.getTransactionByVersion({ txnVersion: txnVersion.toString() });
  }
  ...
  async transactionPending(txnHash: string): Promise<boolean> {
    try {
      const response = await this.client.transactions.getTransactionByHash({ txnHash: txnHash });
  ...
  async getLedgerInfo(): Promise<Gen.IndexResponse> {
    return await this.client.general.getLedgerInfo();
  }

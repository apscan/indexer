...
test("gets genesis resources", async () => {
  ...
  const accountResource = resources.find((r) => lodash.isEqual(r.type, account));
...
test(
  ...
  async () => {
    ...
    const checkAptosCoin = async () => {
      ...
      const account1Resource = resources1.find((r) => lodash.isEqual(r.type, aptosCoin));
      const account2Resource = resources2.find((r) => lodash.isEqual(r.type, aptosCoin));
    ...
    const account2AptosCoin = transactionRes.changes.filter((change) => {
      ...
      return (
        ...
        lodash.isEqual(write.data.type, aptosCoin) &&

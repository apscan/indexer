...
#[async_trait]
pub trait BatchTransactionsProcessor: Send + Sync + Debug {
    /// name of the processor, for status logging
    /// This will get stored in the database for each (`BatchTransactionsProcessor`, transaction_version) pair
    fn name(&self) -> &'static str;

    /// Accepts a transaction, and processes it. This method will be called from `process_transactions_with_status`
    ...
    async fn process_transactions_with_status(
        ...
        self.mark_version_started(transactions[0].version().unwrap());
    ...
    /// Writes that a version has been started for this `BatchTransactionsProcessor` to the DB
    ...
    /// Writes that a version has been completed successfully for this `BatchTransactionsProcessor` to the DB
    ...
    /// Writes that a version has errored for this `BatchTransactionsProcessor` to the DB
    ...
    /// Gets all versions which were not successfully processed for this `BatchTransactionsProcessor` from the DB
    ...
    /// Gets the highest version for this `BatchTransactionsProcessor` from the DB

...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("let sk_bytes = hex::decode(\"{}\").unwrap();", hex::encode(kp.private_key.to_bytes()));
    println!("let pk_bytes = hex::decode(\"{}\").unwrap();", hex::encode(kp.public_key.to_bytes()));
    println!("// signature on TestAptosCrypto(\"test\".to_owned())");
    println!("let sig_bytes = hex::decode(\"{}\").unwrap();", hex::encode(sig.to_bytes()));
}

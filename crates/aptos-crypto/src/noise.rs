...
use crate::{hash::HashValue, hkdf::Hkdf, traits::Uniform as _, ValidCryptoMaterial, x25519};
use sha2::Digest;
use std::{
...
impl NoiseConfig {
    ...
    pub fn initiate_connection(
        ...
        // -> s
        let aead = LessSafeKey::new(UnboundKey::new(&aead::AES_256_GCM, &k[..]).unwrap());
    ...
    pub fn finalize_connection(
        ...
        let nonce = aead::Nonce::assume_unique_for_key([0u8; AES_NONCE_SIZE]);
        let ptxt = aead.open_in_place(nonce, Aad::from(&h), &mut in_out)
            .map_err(|_| {
                // TODO: Do we need to set self.valid = false?
                NoiseError::Decrypt
            })?;
    ...
    > {
        ...
        let nonce = aead::Nonce::assume_unique_for_key([0u8; AES_NONCE_SIZE]);
        let rs = aead.open_in_place(nonce, Aad::from(&h), &mut in_out)
            .map_err(|_| {
                // TODO: Do we need to set self.valid = false?
                NoiseError::Decrypt
            })?;

        let rs = x25519::PublicKey::try_from(rs.as_ref())
...
impl NoiseSession {
    ...
    pub fn write_message_in_place(&mut self, message: &mut [u8]) -> Result<Vec<u8>, NoiseError> {
        ...
        // encrypt in place
        let write_key = LessSafeKey::new(UnboundKey::new(&aead::AES_256_GCM, &self.write_key[..]).unwrap());
        ...
        Ok(authentication_tag.as_ref().try_into().unwrap())
    }
    ...
    pub fn read_message_in_place<'a>(
        ...
        // decrypt in place
        let read_key = LessSafeKey::new(UnboundKey::new(&aead::AES_256_GCM, &self.read_key[..]).unwrap());

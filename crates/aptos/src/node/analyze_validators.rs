...
/// All data needed for new block - NewBlockEvent together with version.
#[derive(Debug, Clone)]
...
impl AnalyzeValidators {
    ...
    pub fn fetch_epoch(epoch: u64, aptos_db: &dyn DbReader) -> Result<Vec<NewBlockInfo>> {
        let batch = 1000;

        let mut cursor = u64::max_value();
        let mut result: Vec<NewBlockInfo> = vec![];
    ...
    pub fn analyze(mut blocks: Vec<NewBlockInfo>, validators: &Vec<AccountAddress>) -> EpochStats {
        assert!(
